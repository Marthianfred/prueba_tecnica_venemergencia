...
import {Reviews} from "@/features/peliculas/tipos";
...
export default function PaginaDetallePelicula() {
    ...
    const {detalle, reparto, reviews} = data;
    ...
    return (
        <main className="min-h-screen bg-[#0a0a0b] text-white">
            ...
            <div className="relative h-[60vh] w-full overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12">
                    <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 items-end">
                        {/* Poster Flotante */}
                        <div
                            className="hidden md:block w-64 aspect-[2/3] relative rounded-xl overflow-hidden shadow-2xl border border-white/10 transform translate-y-12">
                            {posterUrl && (
                                <Image
                                    src={posterUrl}
                                    alt={detalle.title}
                ...
                <span
                    className="flex items-center gap-1 text-yellow-500 font-bold px-2 py-1 bg-yellow-500/10 rounded-md border border-yellow-500/20">
                  ...
                  <Calendar size={16}/> {new Date(detalle.release_date).getFullYear()}
                                ...
                                {detalle.runtime && (
                                    <span
                                        className="flex items-center gap-1 items-center gap-1 px-2 py-1 bg-white/5 rounded-md border border-white/10 text-xs text-white/70">
                     <Clock size={14}/> {detalle.runtime} min
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {detalle.genres?.map((genre) => (
                                        <span key={genre.id}
                                              className="px-3 py-1 bg-white/5 rounded-full border border-white/10 text-xs text-white/70">
                       {genre.name}
                    </span>
                                    ))}
                                </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {reparto.slice(0, 10).map((actor) => (
                            <Link href={`/actor/${actor.id}`} key={actor.id}>
                                <Card
                                    className="h-full hover:bg-white/5 transition-colors group cursor-pointer border-transparent hover:border-white/10">
            ...
            <section className="max-w-7xl mx-auto px-6 py-20">
                <div className="md:ml-[280px]"> {/* Margen para alinear con el contenido a la derecha del poster */}
                    <h3 className="text-2xl font-bold mb-6 flex items-center gap-2">
                        Comentarios de la Crítica <span className="text-indigo-500">★</span>
                    </h3>

                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {reviews.length >0 ? reviews.slice(0, 10).map((review: Reviews) => (
                            <Card
                                className="h-full hover:bg-white/5 transition-colors group cursor-pointer border-transparent hover:border-white/10">
                                <div className="aspect-[2/3] relative rounded-t-xl overflow-hidden bg-white/5">
